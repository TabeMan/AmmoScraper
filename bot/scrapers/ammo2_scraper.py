...
class Ammo2Scraper(BaseScraper):
    """
    A scraper for the 2Ammo website.
    ...
    def __init__(self, url):
        """
        Initializes the Ammo2Scraper with a URL.
    ...
    def extract_product_info(self, row):
        ...
        if row.find("div", {"class": "product details product-item-details"}).find(
            "span", {"class": "rounds-price"}
        ):
            cpr = float(
                row.find("div", {"class": "product details product-item-details"})
                .find("span", {"class": "rounds-price"})
                .text.strip()
                .split(" ")[1]
                .split("p")[0]
                .strip("$")
