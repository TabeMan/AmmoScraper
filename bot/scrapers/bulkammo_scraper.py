...
class BulkammoScraper(BaseScraper):
    """
    A scraper for the Bulk Ammo website.
    ...
    def __init__(self, url):
        """
        Initializes the BulkammoScraper with a URL.
    ...
    def process_page(self, soup):
        ...
        try:
            inner = (
                soup.find("div", {"id": "products-text"})
                .find("div", {"class": "category-products"})
                .find_all("ul", {"class": "products-grid"})
            )
        except Exception as e:
    ...
    def extract_product_info(self, row):
        ...
        if row.find("p", {"class": "special-price"}):
            ...
            original_price = float(
                row.find("span", {"class": "price"}).text.strip().strip("$")
            )
