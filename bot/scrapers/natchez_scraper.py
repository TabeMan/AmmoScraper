...
class NatchezScraper(BaseScraper):
    """
    A scraper for the Natchez Shooting & Outdoors website.
    ...
    def process_page(self, soup):
        ...
        """
        try:
            inner = soup.find_all("div", {"class": "sc-fmdNqN hyACbC"})
        except Exception as e:
    ...
    def extract_product_info(self, row):
        ...
        result["link"] = f"https://www.natchezss.com{link}"
        result["image"] = row.find("img").get("src")
        result["website"] = "Natchez Shooting & Outdoors"

        price_box = row.find("div", {"class": "sc-fXgAZx ZjOLI"}).find_all("span")
        if len(price_box) == 1:
            original_price = float(
                row.find("div", {"class": "sc-gIvpjk gZQWys"})
                .text.strip("$")
                .split("$")[0]
            )
        else:
            original_price = float(
                row.find("div", {"class": "sc-gIvpjk jhIUnK"}).text.strip("$")
            )
